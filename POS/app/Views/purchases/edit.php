<?php $title = 'تعديل فاتورة شراء'; ?>
<h5 class="mb-3">تعديل فاتورة شراء: <?= e($row['invoice_no']) ?></h5>
<form method="post" action="<?= url('/purchases/' . (int)$row['id'] . '/update') ?>" id="purchaseForm">
    <?= csrf_field() ?>
    <input type="hidden" name="items_json" id="purchaseItemsJson">

    <div class="row g-2 mb-3">
        <div class="col-md-4"><label class="form-label">المورد</label><select class="form-select" name="supplier_id" required><?php foreach($suppliers as $s): ?><option value="<?= (int)$s['id'] ?>" <?= (int)$row['supplier_id']===(int)$s['id']?'selected':'' ?>><?= e($s['name']) ?></option><?php endforeach; ?></select></div>
        <div class="col-md-3"><label class="form-label">رقم فاتورة المورد</label><input class="form-control" name="supplier_invoice_no" value="<?= e($row['supplier_invoice_no']) ?>"></div>
        <div class="col-md-2"><label class="form-label">الحالة</label><input class="form-control" value="مسودة" disabled></div>
        <div class="col-md-3"><label class="form-label">طريقة الدفع</label><select class="form-select" name="payment_method_id"><?php foreach($paymentMethods as $m): ?><option value="<?= (int)$m['id'] ?>" <?= (int)$row['payment_method_id']===(int)$m['id']?'selected':'' ?>><?= e($m['name']) ?></option><?php endforeach; ?></select></div>
    </div>
    ...
    <div class="row g-2 mb-2">
        ...
        <div class="col-md-6 d-flex align-items-end"><div class="text-muted small" id="purchaseSearchHint">البحث السريع بالباركود متاح هنا</div></div>
    </div>
    ...
    <div class="row g-2 mb-3">
        <div class="col-md-3"><label class="form-label">المدفوع</label><input type="number" step="0.001" name="paid_total" id="purchasePaid" class="form-control" value="<?= e($row['paid_total']) ?>"></div>
        <div class="col-md-9"><label class="form-label">ملاحظة</label><input class="form-control" name="note" value="<?= e($row['note']) ?>"></div>
    </div>
...
(function(){
    ...
    const rows = <?= json_encode(array_map(static function($i){
        return [
    ...
    }, $row['items']), JSON_UNESCAPED_UNICODE) ?>;
    ...
    rows.forEach((r)=>{
        const op = Array.from(productSel.options).find((o)=> parseInt(o.value, 10) === parseInt(r.product_id, 10));
        const meta = metaFromOption(op);
        if (!r.purchase_unit) {
            r.purchase_unit = defaultUnit(meta);
        }
    });
    rebuildUnitOptions('');
    render();
